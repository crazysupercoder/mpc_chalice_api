...
from chalicelib.libs.purchase.core.values import Id, OrderNumber, SimpleSku, Qty, Cost
from chalicelib.libs.purchase.core.returns import ReturnRequest, ReturnRequestStorageInterface
...
class _ReturnRequestStorageElastic(ReturnRequestStorageInterface):
    ...
    def __restore(self, data: dict) -> ReturnRequest:
        def __restore_delivery_method(descriptor: str) -> ReturnRequest.DeliveryMethod:
            # @todo : refactoring
            ...
            if not method_class:
                raise ValueError('{} does not know, how to work with {} delivery method!'.format(
                    __restore_delivery_method,
                    descriptor
                ))

            return method_class()

        def __restore_refund_method(descriptor: str, extra_data: dict) -> RefundMethodAbstract:
            # @todo : refactoring
        ...
        return_request = self.__reflector.construct(ReturnRequest, {
            ...
            self.__class__.__ENTITY_PROPERTY_DELIVERY_METHOD: __restore_delivery_method(data['delivery_method']),
            self.__class__.__ENTITY_PROPERTY_REFUND_METHOD: __restore_refund_method(
                data['refund_method'],
